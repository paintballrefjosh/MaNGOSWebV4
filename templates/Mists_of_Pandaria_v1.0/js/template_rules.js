var myrules = {
    '#form_tool li' : function(element){
        element.onclick = function(){
            ...
            if(comment && code) {
                ...
                if(code == 'url') {
                    tag = prompt("Enter url: ","../../../Upper2/default.htm");
                    if (tag == null) { return false; }
                    tag = '=' + tag;
                }
                if(code == 'img') {
                    tag1 = prompt("Enter image url: ","../../../Upper2/default.htm");
                    if (tag1 == null) { return false; }
                    selected_text = tag1;
                }
                if(code == 'attach') {
                    window.open(SITE_HREF+'../../../index.php^n=forum&sub=attach&nobody=1.htm','attach','toolbar=no, location=no, directories=no, status=no, resize=yes, menubar=no, scrollbars=yes, width=500, height=600,left=160,top=80');
                    return false; 
                }
                if(code == 'style') {
                ...
                insertAtCursor(comment, '['+code+tag+']'+selected_text+'../../../[/default.htm'+code+']'+spacer);
            }
            return false;
        }
    },
    ...
    '#preview_do' : function(element){
        element.onclick = function(){
            var request = new Ajax.Request(
                SITE_HREF+'../../../index.php^n=ajax&sub=preview&nobody=1&ajaxon=1.htm',
                {
                    method: 'post',
    ...
    '.quote' : function(element){
        element.onclick = function(){
            ...
            req.caching = false;
            req.open('GET', SITE_HREF+'../../../index.php^n=ajax&sub=getquote&nobody=1&ajaxon=1.htm', true);
